...
const AiTtsPreview: React.FC<AiTtsPreviewProps> = ({ text, systemInstruction, voices, onGeneration, language }) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error(language === 'vi'
          ...
          : 'No API key configured. Please open Settings to add your keys.');
      }
      const ai = new GoogleGenAI({ apiKey });
      
      // Clean up system instruction to remove any existing transcript section
      // so the edited text/tags are the only ones the model sees.
      const cleanSystemInstruction = systemInstruction.split(/####?\s*TRANSCRIPT/i)[0].trim();

      const response = await ai.models.generateContent({
        model: "gemini-3.1-flash-tts-preview",
        contents: { parts: [{ text: `${cleanSystemInstruction}\n\n#### TRANSCRIPT\n${text}` }] },
        config: {
      ...
      if (isMountedRef.current) setError(language === 'vi' ? "Tạo lời nói thất bại. Vui lòng thử lại." : "Failed to generate speech. Please try again.");
    } finally {
    ...
    <div className="w-full bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
        ...
        <div className="p-4 flex flex-col sm:flex-row gap-4 items-center justify-between bg-white dark:bg-zinc-800 border-b border-zinc-100 dark:border-zinc-700">
                ...
                >
                    {voices.map(voice => (
                        <option key={voice.name} value={voice.name}>
                            {voice.name} ({language === 'vi' && voice.analysis.gender === 'Female' ? 'Nữ' : language === 'vi' && voice.analysis.gender === 'Male' ? 'Nam' : voice.analysis.gender})
                        </option>
                    ))}
                </select>
            ...
            >
                ...
                <span>{isLoading ? t.generating : isPlaying ? t.stopPreview : (language === 'vi' ? 'Nghe thử' : 'Listen')}</span>
            </button>
        </div>
             ...
             <div className="w-full h-full absolute inset-0 flex items-center justify-center pointer-events-none">
                 {isPlaying ? (
                     ...
                     <div className="flex flex-col items-center gap-2 text-zinc-300 dark:text-zinc-600">
                         <Volume2 size={24} />
                         <span className="text-xs font-medium">{language === 'vi' ? 'Nhấn để nghe thử' : 'Click to preview audio'}</span>
                     </div>
                 )}
             </div>
